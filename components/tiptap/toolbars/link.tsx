...
const LinkToolbar = React.forwardRef<HTMLButtonElement, ButtonProps & { linkBase?: string }>(
  ({ className, linkBase, ...props }, ref) => {
    const { editor } = useToolbar();
    const [link, setLink] = React.useState(() => {
      // Initialize with linkBase if available, otherwise empty string
      return linkBase || '';
    });
    ...
    React.useEffect(() => {
      ...
      if (currentLink) {
        setLink(currentLink);
      } else if (linkBase && typeof linkBase === 'string') {
        setLink(linkBase);
      }
    }, [editor, linkBase]);
