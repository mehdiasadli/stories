...
export default async function UserReadsPage({ params, searchParams }: UserReadsPageProps) {
  ...
  const { chapters, pagination } = await getChaptersOfUser(profileUser.id, 'reads', parsedSearchParams);

  return (
    <>
      {chapters.length === 0 ? (
        <div className='max-w-2xl mx-auto'>
          <p className='text-center text-gray-500'>{profileUser.name} heç bir bölüm oxumayıb</p>
        </div>
      ) : (
        <div>
            ...
            {chapters.map((c) => (
              <div key={c.id}>
                <div className='mb-1 text-xs text-gray-500'>
                  {formatDistanceToNow(new Date(c.reads[0].createdAt))} əvvəl
                </div>
          ...
          <Suspense>
            <Pagination pagination={pagination} userSlug={slug} resource='reads' />
          </Suspense>
        </div>
      )}
    </>
