...
export function ChapterDownloadOptions({ chapter }: ChapterDownloadOptionsProps) {
  ...
  const downloadAsPdf = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('PDF yaradıla bilmədi');
      }
...
<head>
  <meta charset="UTF-8">
  <title>${escapeHtml(chapter.title)} - mahmud</title>
  <style>
...
<body>
  <div class="header">
    ...
    <div class="description"># ${chapter.order} • mahmud • ${escapeHtml(chapter.author.name)}</div>
  </div>
    ...
    <div className='space-y-8'>
      ...
      <div className='space-y-4'>
        ...
        <div className='border border-gray-200 p-6 hover:bg-gray-50 transition-colors'>
          <div className='flex items-center justify-between mb-4'>
            <div>
              <h3 className='text-lg font-serif text-gray-900 mb-1'>sadə mətn</h3>
              <p className='text-sm text-gray-600'>sadə tekst (.txt) faylı, hər bir editora uyğundur</p>
            </div>
          </div>
        ...
        <div className='border border-gray-200 p-6 hover:bg-gray-50 transition-colors'>
          ...
          <button
            ...
            className='w-full py-3 text-sm text-gray-600 hover:text-gray-900 transition-colors border border-gray-200 hover:border-gray-400 bg-white hover:bg-gray-50'
          >
            {isDownloading === 'pdf' ? 'PDF yaradılır...' : 'PDF olaraq yüklə'}
          </button>
        </div>
        ...
        <div className='border border-gray-200 p-6 hover:bg-gray-50 transition-colors'>
          <div className='flex items-center justify-between mb-4'>
            <div>
              <h3 className='text-lg font-serif text-gray-900 mb-1'>HTML sənədi</h3>
              <p className='text-sm text-gray-600'>HTML faylı olaraq yüklə</p>
            </div>
          </div>
          ...
          <button
            ...
            className='w-full py-3 text-sm text-gray-600 hover:text-gray-900 transition-colors border border-gray-200 hover:border-gray-400 bg-white hover:bg-gray-50'
          >
            {isDownloading === 'html' ? 'HTML yüklənir...' : 'HTML olaraq yüklə'}
          </button>
        </div>
      </div>
      ...
      <div className='pt-6 border-t border-gray-200 text-center'>
        <div className='text-sm text-gray-500 space-y-1'>
          <div>bölüm #{chapter.order} • mahmud</div>
          <div>{chapter.author.name}</div>
        </div>
      </div>
    </div>
  );
